...
def train_epoch(model, opt, dataloader, device, lambda_reg, model_type):
    ...
    batch_losses, reg_losses , avg_fail_losses, avg_success_losses = [], [], [], []
    ...
    return losses, reg_loss, avg_fail_loss, avg_success_loss
...
def eval_epoch(model, dataloader_by_split_name, rollouts_by_split_name, device):
    ...
    for split, dataloader in dataloader_by_split_name.items():
        ...
        with torch.no_grad():
            for batch in dataloader:
                img_embeddings = batch["img_embeddings"].to(device)
                action_embeddings = batch["action_embeddings"].to(device)
                valid_masks.append(batch["valid_masks"].to(device))
                labels.append(batch["success_labels"].to(device))
                outputs = model(img_embeddings, action_embeddings)
                scores.append(outputs)
            scores = torch.cat(scores, dim=0).squeeze(-1)
            valid_masks = torch.cat(valid_masks, dim=0) #(dataloader_size, T)
            labels = torch.cat(labels, dim=0) #(dataloader_size,)
    ...
    return logs, classification_logs
