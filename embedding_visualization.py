...
def main(cfg: DictConfig):
    ...
    DatasetHandler = get_dataset_handler(cfg.dataset.name)
    ...
    plt.savefig(f"{save_folder}/{cfg.dataset.name}_feats_vis_skip{feat_skip}-succ.png", bbox_inches='tight')
    plt.close()
    ...
    plt.axis("off"); plt.tight_layout()
    plt.gca().set_aspect('equal', adjustable='box')
    plt.savefig(f"{save_folder}/{cfg.dataset.name}_feats_vis_skip{feat_skip}-taskid.png", bbox_inches='tight')
    plt.close()
